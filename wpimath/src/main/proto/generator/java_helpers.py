...
from wpimath.src.main.proto.generator.lib import MessageClass, ProtobufModule, MessageField
...
def get_schema(message : MessageClass):
    schema_fields = []
...
def get_struct_size(message : MessageClass):
    num_doubles = 0
    num_ints = 0
...
def get_protobuf_unpack(message : MessageClass):
    output_fields = []
...
def render_message_java(module : ProtobufModule, message : MessageClass):
    ...
    env.globals["struct_unpack"] = struct_unpack
    env.globals["struct_pack"] = struct_pack
    env.globals["get_size"] = get_struct_size
    ...
    proto_src = os.path.join(wpimath_src_dir, f"{module.subfolder}/{lang_type}ProtoSerde.java")
    with open(proto_src, "w") as f:
        f.write(env.get_template("java_proto_serde.jinja2").render(**kwargs))

    struct_src = os.path.join(wpimath_src_dir, f"{module.subfolder}/{lang_type}StructSerde.java")
