...
const Transform3d kExpectedData{Translation3d(1.91_m, 2.29_m, 1.74_m), Rotation3d(1.1_rad, 2.2_rad, 3.3_rad)};
}  // namespace
...
TEST(Transform3dTest, ProtobufPack) {
  ...
  ProtoType::Pack(&proto, kExpectedData);
    ...
    EXPECT_EQ(kExpectedData.Translation().Z().value(), proto.translation().z_meters());
    EXPECT_EQ(kExpectedData.Rotation().GetQuaternion().W(), proto.rotation().q().w());
    EXPECT_EQ(kExpectedData.Rotation().GetQuaternion().X(), proto.rotation().q().x());
    EXPECT_EQ(kExpectedData.Rotation().GetQuaternion().Y(), proto.rotation().q().y());
    EXPECT_EQ(kExpectedData.Rotation().GetQuaternion().Z(), proto.rotation().q().z());
}
...
TEST(Transform3dTest, ProtobufUnpack) {
  ...
  proto.mutable_translation()->set_z_meters(kExpectedData.Translation().Z().value());
  proto.mutable_rotation()->mutable_q()->set_w(kExpectedData.Rotation().GetQuaternion().W());
  proto.mutable_rotation()->mutable_q()->set_x(kExpectedData.Rotation().GetQuaternion().X());
  proto.mutable_rotation()->mutable_q()->set_y(kExpectedData.Rotation().GetQuaternion().Y());
  proto.mutable_rotation()->mutable_q()->set_z(kExpectedData.Rotation().GetQuaternion().Z());
