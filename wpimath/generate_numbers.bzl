...
def _java_nat_file_gen_impl(ctx):
    nat_out = ctx.actions.declare_file("src/main/edu/wpi/first/wpiutil/math/Nat.java")

    content = "package edu.wpi.first.wpiutil.math;\n"
    for i in range(__NUM_NUMS + 1):
        content += "import edu.wpi.first.wpiutil.math.numbers.N" + str(i) + ";\n"
...
"""
    ...
    for i in range(__NUM_NUMS + 1):
        file_out = ctx.actions.declare_file("src/main/edu/wpi/first/wpiutil/math/numbers/N" + str(i) + ".java")
