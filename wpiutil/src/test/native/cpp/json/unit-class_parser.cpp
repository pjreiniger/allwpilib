...
class SaxCountdown : public wpi::json::json_sax_t
{
  public:
...
{
    json j;
    json::parser(wpi::detail::input_adapter(s)).parse(true, j);

    // if this line was reached, no exception occurred
    // -> check if result is the same without exceptions
    json j_nothrow;
    CHECK_NOTHROW(json::parser(wpi::detail::input_adapter(s), nullptr, false).parse(true, j_nothrow));
    CHECK(j_nothrow == j);

    json j_sax;
    wpi::detail::json_sax_dom_parser<json> sdp(j_sax);
...
bool accept_helper(const std::string& s)
{
    ...
    json j;
    CHECK_NOTHROW(json::parser(wpi::detail::input_adapter(s), nullptr, false).parse(true, j));
    const bool ok_noexcept = !j.is_discarded();

    // 2. accept s
    const bool ok_accept = json::parser(wpi::detail::input_adapter(s)).accept(true);
    ...
    CHECK(json::parser(wpi::detail::input_adapter(s)).accept(false) == !el.errored);
...
TEST(ParserClassParseTest, Null)
{
    CHECK(parser_helper("null") == json(nullptr));
}

TEST(ParserClassParseTest, True)
{
    CHECK(parser_helper("true") == json(true));
}

TEST(ParserClassParseTest, False)
{
...
TEST(ParserClassParseArrayTest, EmptyArray)
{
    CHECK(parser_helper("[]") == json(json::value_t::array));
    CHECK(parser_helper("[ ]") == json(json::value_t::array));
}

TEST(ParserClassParseArrayTest, NonemptyArray)
{
...
TEST(ParserClassParseObjectTest, EmptyObject)
{
    CHECK(parser_helper("{}") == json(json::value_t::object));
    CHECK(parser_helper("{ }") == json(json::value_t::object));
}

TEST(ParserClassParseObjectTest, NonemptyObject)
{
...
TEST(ParserClassParseStringTest, Empty)
{
...
TEST(ParserClassParseStringErrorsTest, BasicTest)
{
    // error: tab in string
...
TEST(ParserClassParseStringErrorsTest, AdditionalTestForNullByte)
{
...
TEST(ParserClassParseStringTest, Escaped)
{
    // quotation mark "\""
    auto r1 = R"("\"")"_json;
...
TEST(ParserClassParseNumberIntegersTest, WithoutExponent)
{
...
TEST(ParserClassParseNumberIntegersTest, WithExponent)
{
...
TEST(ParserClassParseNumberIntegersTest, EdgeCases)
{
...
TEST(ParserClassParseNumberIntegersTest, OverTheEdgeCases)
{
...
TEST(ParserClassParseNumberFloatingPointTest, WithoutExponent)
{
...
TEST(ParserClassParseNumberFloatingPointTest, WithExponent)
{
...
TEST(ParserClassParseNumberTest, Overflow)
{
...
TEST(ParserClassParseNumberTest, InvalidNumbers)
{
...
TEST(ParserClassAcceptTest, Null)
{
    CHECK(accept_helper("null"));
}

TEST(ParserClassAcceptTest, True)
{
    CHECK(accept_helper("true"));
}

TEST(ParserClassAcceptTest, False)
{
...
TEST(ParserClassAcceptArrayTest, EmptyArray)
{
    CHECK(accept_helper("[]"));
    CHECK(accept_helper("[ ]"));
}

TEST(ParserClassAcceptArrayTest, NonemptyArray)
{
...
TEST(ParserClassAcceptObjectTest, EmptyObject)
{
    CHECK(accept_helper("{}"));
    CHECK(accept_helper("{ }"));
}

TEST(ParserClassAcceptObjectTest, NonemptyObject)
{
...
TEST(ParserClassAcceptStringTest, Empty)
{
    CHECK(accept_helper("\"\""));
}

TEST(ParserClassAcceptStringTest, Errors)
{
    // error: tab in string
    CHECK(accept_helper("\"\t\"") == false);
    // error: newline in string
...
TEST(ParserClassAcceptStringTest, Escaped)
{
    // quotation mark "\""
    auto r1 = R"("\"")"_json;
    CHECK(accept_helper("\"\\\"\""));
...
TEST(ParserClassAcceptNumberIntegersTest, WithoutExponent)
{
...
TEST(ParserClassAcceptNumberIntegersTest, WithExponent)
{
...
TEST(ParserClassAcceptNumberIntegersTest, EdgeCases)
{
    // From RFC8259, Section 6:
    // Note that when such software is used, numbers that are
    // integers and are in the range [-(2**53)+1, (2**53)-1]
...
TEST(ParserClassAcceptNumberIntegersTest, OverTheEdgeCases)
{
    // While RFC8259, Section 6 specifies a preference for support
    // for ranges in range of IEEE 754-2008 binary64 (double precision)
    // this does not accommodate 64 bit integers without loss of accuracy.
...
TEST(ParserClassAcceptNumberFloatingPointTest, WithoutExponent)
{
...
TEST(ParserClassAcceptNumberFloatingPointTest, WithExponent)
{
...
TEST(ParserClassAcceptNumberTest, Overflow)
{
    // overflows during parsing
    CHECK(!accept_helper("1.18973e+4932"));
}

TEST(ParserClassAcceptNumberTest, InvalidNumbers)
{
...
TEST(ParserClassTest, ParseErrors)
{
...
TEST(ParserClassTest, ParseErrorsAccept)
{
    // unexpected end of number
    CHECK(accept_helper("0.") == false);
    CHECK(accept_helper("-") == false);
    ...
    for (int c = 1; c < 128; ++c)
    {
        ...
        switch (c)
        {
            ...
            case ('t'):
            {
                CHECK(json::parser(wpi::detail::input_adapter(s)).accept());
                break;
            }
            ...
            default:
            {
                CHECK(json::parser(wpi::detail::input_adapter(s)).accept() == false);
                break;
            }
        }
    }
    ...
    // invalid \uxxxx escapes
    {
        ...
        for (int c = 1; c < 128; ++c)
        {
            ...
            {
                CAPTURE(s1)
                CHECK(json::parser(wpi::detail::input_adapter(s1)).accept());
                CAPTURE(s2)
                CHECK(json::parser(wpi::detail::input_adapter(s2)).accept());
                CAPTURE(s3)
                CHECK(json::parser(wpi::detail::input_adapter(s3)).accept());
                CAPTURE(s4)
                CHECK(json::parser(wpi::detail::input_adapter(s4)).accept());
            }
            else
            {
                CAPTURE(s1)
                CHECK(json::parser(wpi::detail::input_adapter(s1)).accept() == false);

                CAPTURE(s2)
                CHECK(json::parser(wpi::detail::input_adapter(s2)).accept() == false);

                CAPTURE(s3)
                CHECK(json::parser(wpi::detail::input_adapter(s3)).accept() == false);

                CAPTURE(s4)
                CHECK(json::parser(wpi::detail::input_adapter(s4)).accept() == false);
            }
        }
    }
...
TEST(ParserClassTest, TestsFoundByMutate)
{
...
class ParserClassCallbackFunctionTest : public ::testing::Test {
 protected:
    const char* s_object = R"(
        {
            "foo": 2,
            "bar": {
                "baz": 1
    ...
    const char* s_array = R"(
        [1,2,[3,4,5],4,5]
    )";

    const char* structured_array = R"(
        [
            1,
            {
                 "foo": "bar"
...
TEST_F(ParserClassCallbackFunctionTest, FilterNothing)
{
...
TEST_F(ParserClassCallbackFunctionTest, FilterEverything)
{
    json j_object = json::parse(s_object, [](int /*unused*/, json::parse_event_t /*unused*/, const json& /*unused*/) noexcept
    {
        return false;
...
TEST_F(ParserClassCallbackFunctionTest, FilterSpecificElement)
{
...
TEST_F(ParserClassCallbackFunctionTest, FilterObjectInArray)
{
...
class ParserClassCallbackFunctionFilterSpecificEventsTest : public ParserClassCallbackFunctionTest{};


TEST_F(ParserClassCallbackFunctionFilterSpecificEventsTest, FirstClosingEvent)
{
    {
...
TEST_F(ParserClassCallbackFunctionTest, SpecialCases)
{
...
TEST(ParserClassConstructingFromContiguousContainersTest, FromStdVector)
{
    std::vector<uint8_t> v = {'t', 'r', 'u', 'e'};
    json j;
    json::parser(wpi::detail::input_adapter(std::begin(v), std::end(v))).parse(true, j);
    CHECK(j == json(true));
}

TEST(ParserClassConstructingFromContiguousContainersTest, FromStdArray)
{
...
TEST(ParserClassConstructingFromContiguousContainersTest, FromArray)
{
...
TEST(ParserClassConstructingFromContiguousContainersTest, FromCharLiteral)
{
    CHECK(parser_helper("true") == json(true));
}

TEST(ParserClassConstructingFromContiguousContainersTest, FromStdString)
{
...
TEST(ParserClassConstructingFromContiguousContainersTest, FromStdInitializerList)
{
...
TEST(ParserClassConstructingFromContiguousContainersTest, FromStdValarray)
{
...
TEST(ParserClassImproveTestCoverageTest, ParserWithCallback)
{
    json::parser_callback_t cb = [](int /*unused*/, json::parse_event_t /*unused*/, json& /*unused*/) noexcept
    {
        return true;
...
TEST(ParserClassImproveTestCoverageSAXParserTest, WithoutValue)
{
    SaxCountdown s(1);
    CHECK(json::sax_parse("{}", &s) == false);
}

TEST(ParserClassImproveTestCoverageSAXParserTest, WithValue)
{
    SaxCountdown s(3);
    CHECK(json::sax_parse("{\"k1\": true}", &s) == false);
}

TEST(ParserClassImproveTestCoverageSAXParserTest, SecondKey)
{
    SaxCountdown s(3);
    CHECK(json::sax_parse("{\"k1\": true, \"k2\": false}", &s) == false);
}

TEST(ParserClassImproveTestCoverageSAXParserTest, WithoutValue2)
{
    SaxCountdown s(1);
    CHECK(json::sax_parse("[]", &s) == false);
}

TEST(ParserClassImproveTestCoverageSAXParserTest, WithValue2)
{
    SaxCountdown s(2);
    CHECK(json::sax_parse("[1]", &s) == false);
}

TEST(ParserClassImproveTestCoverageSAXParserTest, Float)
{
    SaxCountdown s(0);
    CHECK(json::sax_parse("3.14", &s) == false);
}

TEST(ParserClassImproveTestCoverageSAXParserTest, False)
{
    SaxCountdown s(0);
    CHECK(json::sax_parse("false", &s) == false);
}

TEST(ParserClassImproveTestCoverageSAXParserTest, Null)
{
    SaxCountdown s(0);
    CHECK(json::sax_parse("null", &s) == false);
}

TEST(ParserClassImproveTestCoverageSAXParserTest, True)
{
    SaxCountdown s(0);
    CHECK(json::sax_parse("true", &s) == false);
}

TEST(ParserClassImproveTestCoverageSAXParserTest, Unsigned)
{
    SaxCountdown s(0);
    CHECK(json::sax_parse("12", &s) == false);
}

TEST(ParserClassImproveTestCoverageSAXParserTest, Integer)
{
    SaxCountdown s(0);
    CHECK(json::sax_parse("-12", &s) == false);
}

TEST(ParserClassImproveTestCoverageSAXParserTest, String)
{
    SaxCountdown s(0);
    CHECK(json::sax_parse("\"foo\"", &s) == false);
}

TEST(ParserClassTest, ErrorMessagesForComments)
{
    json _;
