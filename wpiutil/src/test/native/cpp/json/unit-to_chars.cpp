...
#include "unit-json.h"
using wpi::detail::dtoa_impl::reinterpret_bits;
...
TEST(DigitGenTest, SinglePrecision)
{
    auto check_float = [](float number, const std::string & digits, int expected_exponent)
    {
        ...
        int len = 0;
        int exponent = 0;
        wpi::detail::dtoa_impl::grisu2(buf.data(), len, exponent, number);
...
TEST(DigitGenTest, DoublePrecision)
{
...
TEST(FormattingTest, SinglePrecision)
{
    auto check_float = [](float number, const std::string & expected)
    {
        std::array<char, 33> buf{};
        char* end = wpi::detail::to_chars(buf.data(), buf.data() + 32, number); // NOLINT(cppcoreguidelines-pro-type-vararg,hicpp-vararg)
...
TEST(FormattingTest, DoublePrecision)
{
