...
static_assert(noexcept(wpi::to_json(*j, 2)), "");
static_assert(noexcept(wpi::to_json(*j, 2.5)), "");
static_assert(noexcept(wpi::to_json(*j, true)), "");
static_assert(noexcept(wpi::to_json(*j, test{})), "");
static_assert(noexcept(wpi::to_json(*j, pod{})), "");
static_assert(!noexcept(wpi::to_json(*j, pod_bis{})), "");
...
TEST(RuntimeChecksTest, NothrowCopyConstructibleExceptions)
{
...
TEST(RuntimeChecksTest, SilenceWunneededInternalDeclarationErrors)
{
    j = nullptr;
    json j2;
    to_json(j2, pod());
